...
public class CommentCustomRepositoryImpl implements CommentCustomRepository {
    ...
    public List<CommentDto> findAllByPostId(UserId userId, PostId postId) {
        List<CommentDto> result = em.createQuery(
                        "select new com.monkey.domain.comment.root.dto.CommentDto(p.userId.id, c.userId.id, u.name, c.id, c.content, c.createdAt, c.hasReply, c.isSecrete) " +
    ...
    public List<CommentDto> findAllByRefComment(UserId userId, CommentId commentId) {
        List<CommentDto> result = em.createQuery(
                        "select new com.monkey.domain.comment.root.dto.CommentDto(c2.userId.id, u.id, u.name, c1.id, c1.content, c1.createdAt, c1.hasReply, c1.isSecrete) " +
