...
import com.monkey.domain.comment.dto.CommentDto;
import com.monkey.domain.post.entity.PostId;
import com.monkey.domain.post.dto.PostDto;
...
public class PostCustomRepositoryImpl implements PostCustomRepository {
    ...
    public PostRes getResponseByPostId(PostId postId) {
        PostDto postDto = Optional.of(em.createQuery(
                        "select new com.monkey.domain.post.dto.PostDto(p.content, p.createdAt, p.modifiedAt) " +
                                "from Post p " +
                                "where p.id = :postId",
                        PostDto.class)
        ...
        List<CommentDto> commentDtoList =
                em.createQuery(
                                "select new com.monkey.domain.comment.dto.CommentDto(c.content, c.createAt, c.hasReply) " +
                                        "from Comment c " +
